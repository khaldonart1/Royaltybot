...
async def reconcile_all_referrals_job(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    verified_ids = {u['user_id'] for u in all_users if u.get('is_verified')}
    all_referrer_ids = {m['referrer_user_id'] for m in all_mappings}
    ...
    for user in all_users:
        ...
        if (user.get('real_referrals') != current_counts['real'] or 
            user.get('fake_referrals') != current_counts['fake']):
...
async def recheck_leavers_and_notify_job(context: ContextTypes.DEFAULT_TYPE) -> None:
    owner_id = context.job.chat_id
    await context.bot.send_message(owner_id, "⏳ جاري بدء فحص شامل (للمغادرين وغيرهم)...")
            ...
            if 'referrer_id' in context.user_data:
                ...
                try:
                    ...
                    if referrer_db:
                        ...
                        await upsert_user_in_db({
                            'user_id': referrer_id,
                            'real_referrals': new_real,
                            'fake_referrals': new_fake
                        })
...
CALLBACK_DISPATCHER: Dict[str, Callable[[CallbackQuery, ContextTypes.DEFAULT_TYPE], Awaitable[None]]] = {
    ...
    Callback.ADMIN_GET_USER_REFERRALS.value: handle_get_user_referrals_request,
...
async def handle_admin_messages(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if state == State.AWAITING_REFERRAL_LIST_USER_ID:
        try:
            ...
            await update.message.reply_text(f"⏳ جارٍ جلب قائمة الإحالات الحقيقية للمستخدم {target_user.get('full_name')}...")
            ...
            if not real_referral_ids:
                await update.message.reply_text(f"المستخدم **{target_user.get('full_name')}** ليس لديه أي إحالات حقيقية.", parse_mode=ParseMode.MARKDOWN, reply_markup=get_admin_panel_keyboard())
                return
            
            
            user_map = {u['user_id']: u for u in all_users}
            
            response_text = f"✅ **قائمة الإحالات الحقيقية للمستخدم {target_user.get('full_name')} ({len(real_referral_ids)}):**\n\n"
            for ref_id in real_referral_ids:
                ref_user_info = user_map.get(ref_id)
                if ref_user_info:
                    full_name = ref_user_info.get('full_name', 'اسم غير معروف')
                    response_text += f"- **الاسم:** {full_name}\n  - **ID:** `{ref_id}`\n"
                else:
                    response_text += f"- **ID:** `{ref_id}` (لا تتوفر معلومات إضافية)\n"
...
def main() -> None:
    if not Config.BOT_TOKEN or "YOUR_BOT_TOKEN" in Config.BOT_TOKEN or \
       not Config.SUPABASE_URL or "YOUR_SUPABASE_URL" in Config.SUPABASE_URL:
        logger.critical("FATAL: Bot token or Supabase credentials are not configured in the Config class.")
        return
